...
public class RpiSensorViaMCP3008 {
	...
	public float getTemperature(GpioController gpio) {
...
public int getAmbient(GpioController gpio) {
		System.out.println("Entering in getAmbient method");
		//GpioController gpio = GpioFactory.getInstance();
		MCP3008ADCReader.initSPI(gpio);
		System.out.println("Entering in getAmbient method2222");
...
*/
	...
	public static void main(String[] args) {
		...
		while (keepRunning) {
			/*
			 * Reading ambient light from the photocell sensor using the MCP3008
			 * ADC
			 */
			int adc_ambient = MCP3008ADCReader
					.readAdc(MCP3008ADCReader.MCP3008_input_channels.CH1.ch());
			...
			if (DEBUG) {
				System.out
						.println("readAdc:"
								...
								+ " (0x"
								+ MCP3008ADCReader.lpad(
										Integer.toString(adc_ambient, 16)
												.toUpperCase(), "0", 2)
								+ ", 0&"
								+ MCP3008ADCReader.lpad(
										Integer.toString(adc_ambient, 2), "0",
										8) + ")");
				System.out.println("Ambient:" + ambient + "/100 ("
						+ adc_ambient + "/1024)");
			}
			...
			if (DEBUG) {
				System.out.println("readAdc:"
						...
						+ ", 0&"
						+ MCP3008ADCReader.lpad(
								Integer.toString(adc_temperature, 2), "0", 8)
						+ ")");
				System.out.println("Temperature:" + temperature + "/100 ("
						+ adc_temperature + "/1024)");
			}
			...
			System.out.println("Ambient:" + ambient + "/100; Temperature:"
			+ temperature + "/100 => " + String.valueOf(temp_C)
					+ "C => " + String.valueOf(temp_F) + "F");
